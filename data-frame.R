...
qplot(data = df, x=df$Income.Group, y=df$Birth.rate, size = I(3),
...
qplot(data= df, x= Internet.users, y= Birth.rate, colour= I('orange'), size= I(4))

qplot(data= df, x= Internet.users, y= Birth.rate, colour= Income.Group, size= I(4))

# --------------- Creating Data Frames -----------------
# For this section one needs to upload the following vectors:
...
head(df, n=2)
head(mydf, n=2)
merged_df <- merge(df, mydf, by.x = 'Country.Code', by.y = 'Code')
head(merged_df, n=2)
...
qplot(data = merged_df, x = Internet.users, y = Birth.rate, size = I(4),
      colour = Region, shape = I(20))

# Transparency
qplot(data = merged_df, x = Internet.users, y = Birth.rate, size = I(4),
      colour = Region, shape = I(20), alpha=I(0.5))
# Title
qplot(data = merged_df, x = Internet.users, y = Birth.rate, size = I(4),
      colour = Region, shape = I(20),
